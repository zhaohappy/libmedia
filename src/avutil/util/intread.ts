...
export function rl32(p: pointer<void>) {
  return (rl16(p) << 16) | rl16(reinterpret_cast<pointer<uint8>>(p) + 2)
}

export function rb32(p: pointer<void>) {
  return (rb16(reinterpret_cast<pointer<uint8>>(p) + 2) << 16) | rb16(p)
}

export function rl64(p: pointer<void>) {
  return (BigInt(rl32(p)) << 32n) | BigInt(rl32(reinterpret_cast<pointer<uint8>>(p) + 4))
}

export function rb64(p: pointer<void>) {
  return (BigInt(rb32(reinterpret_cast<pointer<uint8>>(p) + 4)) << 32n) | BigInt(rb32(p))
}