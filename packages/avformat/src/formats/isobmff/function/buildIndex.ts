...
export function buildIndex(stream: AVStream, isobmffContext: IsobmffContext) {
  ...
  if (!isobmffContext.ignoreEditlist && stream.metadata[AVStreamMetadataKey.ELST]?.length) {
    let timeOffset = 0n
    ...
    if ((emptyDuration || startTime) && isobmffContext.timescale > 0) {
      if (emptyDuration) {
        emptyDuration = avRescaleQ(emptyDuration, { num: 1, den: isobmffContext.timescale }, stream.timeBase)
