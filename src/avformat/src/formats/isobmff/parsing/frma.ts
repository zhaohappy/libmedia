...
import { AVMediaType, type AVStream } from '@libmedia/avutil'
...
export default async function read(ioReader: IOReader, stream: AVStream, atom: Atom, isobmffContext: IsobmffContext) {
  ...
  let codecId = tags.codecIsobmffAudioTags[format]
  ...
  if (codecId > 0 && stream.codecpar.codecType !== AVMediaType.AVMEDIA_TYPE_VIDEO) {
    ...
    codecId = tags.codecIsobmffVideoTags[format]
    if (codecId > 0) {
      stream.codecpar.codecType = AVMediaType.AVMEDIA_TYPE_VIDEO
    }
    else {
      codecId = tags.codecIsobmffSubtiteTags[format]
