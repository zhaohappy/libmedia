  ...
  if (version > 1) {
    ...
    while (total) {
      ...
      if (type === 'dimg') {
        ...
        const item = movContext.heif.items?.find((item) => item.id === fromItemId)
        if (item
          ...
          if (!movContext.heif.grid) {
            movContext.heif.grid = []
          }
          movContext.heif.grid.push({
            item,
            tileIdList
          })
        }
        ...
        if (entries > 1) {
          ...
          const item = movContext.heif.items?.find((item) => item.id === toItemId)
          if (item) {
            if (!item.refs) {
              item.refs = {}
            }
