...
import { is, text, object } from '@libmedia/common'
...
export default function write(ioWriter: IOWriterSync, stream: AVStream, isobmffContext: IsobmffContext) {
  ...
  ioWriter.writeString(isobmffContext.useMetadataTags ? 'mdta' : 'mdir')
  ioWriter.writeUint32(0)
  ioWriter.writeUint32(0)
  ioWriter.writeUint32(0)
  ...
  function getKey(key: string) {
    if (isobmffContext.useMetadataTags) {
      keys.push(key)
      return keys.length
    }
    else {
  ...
  if (isobmffContext.covr) {
    tags.push({
      key: getKey('covr'),
      value: isobmffContext.covr.data,
      type: isobmffContext.covr.type
    })
  }

  if (isobmffContext.metadata) {
    object.each(isobmffContext.metadata, (value, key) => {
