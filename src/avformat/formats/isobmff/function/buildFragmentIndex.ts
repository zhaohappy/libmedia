/*
 * libmedia isobmff fragment indexes
 *
 * 版权所有 (C) 2024 赵高兴
...
export function buildFragmentIndex(stream: Stream, track: FragmentTrack, isobmffContext: IsobmffContext, pos: int64, ioFlag: int32 = 0) {
  ...
  if (track.baseIsMoof || currentOffset < pos) {
    currentOffset += isobmffContext.currentFragment.pos
  }
  ...
  const cenc = isobmffContext.cencs ? isobmffContext.cencs[context.trackId] : null
  if (cenc && track.cenc) {
