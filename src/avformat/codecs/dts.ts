...
const DTSSampleRateTab = [0, 8000, 16000, 32000, 0, 0, 11025, 22050, 44100, 0, 0, 12000, 24000, 48000, 0, 0]
...
export function parseHeader(buf: Uint8ArrayInterface) {
  ...
  if (info.syncWord !== 0x7ffe8001) {
    return -1
  }

  info.frameType = bitReader.readU1()
  info.sampleCount = bitReader.readU(5)
  info.crcFlag = bitReader.readU1()
  info.sampleBlock = bitReader.readU(7)
  info.frameSize = bitReader.readU(14)
