...
export default async function read(ioReader: IOReader, stream: AVStream, atom: Atom, isobmffContext: IsobmffContext) {
  ...
  let axis = await ioReader.readUint8() & 0x1

  if (isobmffContext.heif?.currentItem) {
    isobmffContext.heif.currentItem.hflip = !!axis
    isobmffContext.heif.currentItem.vflip = !axis
  }
  ...
  if (remainingLength > 0) {
    ...
    logger.error(`read imir error, size: ${atom.size}, read: ${atom.size - remainingLength}`)
  }
}
