...
export default class OIsobmffFormat extends OFormat {
  ...
  private enableStreams(formatContext: AVOFormatContext) {
    ...
    array.each(formatContext.streams, (stream, index) => {
      ...
      if (stream.disposition & AVDisposition.DEFAULT) {
        ...
        (stream.privData as IsobmffStreamContext).flags |= TKHDFlags.ENABLED
      }
    })
    ...
    for (let i = 0; i < AVMediaType.AVMEDIA_TYPE_NB; i++) {
      switch (i) {
        ...
        case AVMediaType.AVMEDIA_TYPE_SUBTITLE:
          if (enabled[i] > 1) {
            (formatContext.streams[enabled[i]].privData as IsobmffStreamContext).perStreamGrouping = true
          }
          if (!enabled[i] && first[i] >= 0) {
            (formatContext.streams[first[i]].privData as IsobmffStreamContext).flags |= TKHDFlags.ENABLED
          }
          break
      }
    }
  ...
  public writeHeader(formatContext: AVOFormatContext): number {
    ...
    if (this.options.mp4Mode === Mp4Mode.MOV) {
    ...
    oisobmff.writeFtyp(formatContext.ioWriter, this.context)
    ...
    if (this.options.fragment) {
      ...
      oisobmff.writeMoov(formatContext.ioWriter, formatContext, this.context)
  ...
  private updateCurrentChunk(formatContext: AVOFormatContext) {
    ...
    const prevIsobmffStreamContext = prevStream.privData as IsobmffStreamContext
    prevIsobmffStreamContext.chunkCount++
    prevIsobmffStreamContext.chunkOffsets.push(currentChunk.pos)
    if (!prevIsobmffStreamContext.stscFirstChunk.length) {
      prevIsobmffStreamContext.stscFirstChunk.push(prevIsobmffStreamContext.chunkCount)
      prevIsobmffStreamContext.stscSamplesPerChunk.push(currentChunk.sampleCount)
      prevIsobmffStreamContext.stscSampleDescriptionIndex.push(1)
      prevIsobmffStreamContext.lastStscCount = currentChunk.sampleCount
    }
    else {
      if (prevIsobmffStreamContext.lastStscCount !== currentChunk.sampleCount) {
        prevIsobmffStreamContext.stscFirstChunk.push(prevIsobmffStreamContext.chunkCount)
        prevIsobmffStreamContext.stscSamplesPerChunk.push(currentChunk.sampleCount)
        prevIsobmffStreamContext.stscSampleDescriptionIndex.push(1)
        prevIsobmffStreamContext.lastStscCount = currentChunk.sampleCount
  ...
  private updateCurrentFragment(formatContext: AVOFormatContext, currentDts?: int64) {
    if (this.context.currentFragment.firstWrote) {
      ...
      oisobmff.writeMoof(formatContext.ioWriter, formatContext, this.context)
  ...
  public writeTrailer(formatContext: AVOFormatContext): number {
    if (!this.context.fragment) {
      ...
      if (this.options.hasTfra) {
        oisobmff.writeMfra(formatContext.ioWriter, formatContext, this.context)
      }
      else {
